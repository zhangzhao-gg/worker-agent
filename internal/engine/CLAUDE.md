...
engine.go: 入口 struct，持有 db/cityAPI/llm/router/workerName，暴露 Run() + RunConversation()，RunContext 含 PersistentMemories，生成 sessionID 并注入推理日志回调
prompt.go: system prompt 动态组装，soul（含 body_status）+ 持久记忆 + context → 完整 prompt；buildConversationPrompt 为对话模式精简 prompt
...
loop.go: agent loop 推理循环，每轮写 reasoning_logs + 首次 stop 注入审视提示（review 机制）+ 自动压缩 + MaxRounds 支持
