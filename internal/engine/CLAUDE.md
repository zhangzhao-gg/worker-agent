...
engine.go: 入口 struct，持有 db/cityAPI/llm/router/workerName，暴露 Run() + RunConversation()，RunContext 含 PersistentMemories，生成 sessionID 并注入推理日志回调，对话入口提示回复前先处理必要记忆，LLM 失败时回复并结束对话释放等待方
